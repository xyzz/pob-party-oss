...
local function do_return()
  while true do
    if stack.top <= 0 then
      ...
      local top = get_top()
      io.stdout:write(("ERROR (%4d, line %7d): tried to return above end of stack from function defined at %s:%d-%d\n"):
...
function profile.record(a, b, c, d)
  ...
  elseif a == "Y" then                          -- Yield
    if thread_stack.top <= 0 then
      error_count = error_count + 1
      local top = get_top()
      io.stdout:write(("ERROR (%4d, line %7d): tried to yield to unknown thread from function defined at %s:%d-%d\n"):
  ...
  elseif a == "E" then                          -- Error!
    ...
    if not top then
      error_count = error_count + 1
      io.stdout:write(("ERROR (%4d, line %7d): recorded execution of %g microseconds at line %d in an unknown stack frame at height %d\n")
      ...
      if top.func.line_defined > 0 and
        ...
        if not above_top or
          ...
          error_count = error_count + 1
          io.stdout:write(("ERROR (%4d, line %7d): counted execution of %g microseconds at line %d of a function defined at %s:%d-%d\n"):
...
function profile.close()
  ...
  for _, l in ipairs(lines) do
    if l.child_time > total_time then
      error_count = error_count + 1
      io.stdout:write(("ERROR (%4d): Line %s:%d accumulated child execution time of %g microseconds which is more that the total running time of %g microseconds: %s\n"):
        format(error_count, l.file.filename, l.line_number, l.child_time, total_time, l.text or "-"))
    elseif l.child_time < 0 then
      error_count = error_count + 1
      io.stdout:write(("ERROR (%4d): Line %s:%d accumulated a negative child execution time (%g microseconds): %s\n"):
  ...
  local function number_format(title, fmt, max_value)
    ...
    max_len = math.max(title:len(), max_len) + 2
  ...
  io.stdout:write(("Total time "..formats.n.time.." %s\n"):format(total_time / formats.divisor, formats.time_units))
  io.stdout:write("Times in ", formats.time_units, "\n")

  local function report_format(f) return title_format.."  "..f.hit.."  "..f.time.."  "..f.time.."  "..f.time.. " | %s\n" end
  line_format = report_format(formats.n)

  io.stdout:write("Top 20 lines by total time\n")
  io.stdout:write(report_format(formats.s):format("File:line", "Hits", "Total", "Self", "Child", "Line"))
  for i = 1, math.min(20, #lines) do
    local l = lines[i]
    io.stdout:write(line_format:format(l.title, l.hits,
  ...
  io.stdout:write(report_format(formats.s):format("File:lines", "Hits", "Total", "Self", "Child", "Line"))
  for i = 1, math.min(20, #f2) do
    local l = f2[i]
